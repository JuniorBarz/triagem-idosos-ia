...
def analisar():
    ...
    salvar_dados(nome, idade, sintomas, resultado)

# Função para salvar os dados em CSV
def salvar_dados(nome, idade, sintomas, resultado):
    data = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open("registro_saude.csv", mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([data, nome, idade, ", ".join(sintomas), resultado])

# Interface principal
root = tk.Tk()
root.title("Triagem de Saúde - Versão 2")
