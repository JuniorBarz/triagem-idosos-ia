...
# Salva histórico por paciente em CSV separado
...
# Conta quantas triagens o paciente já tem
...
# Gera gráfico com frequência de sintomas
def ver_grafico():
    nome = nome_entry.get()
    if not nome:
    ...
    plt.bar(sintomas_freq.keys(), sintomas_freq.values())
    plt.title(f"Frequência de sintomas - {nome}")
    plt.xlabel("Sintomas")
...
# Interface principal
root = tk.Tk()
root.title("Triagem de Saúde - Versão 6")
...
tk.Button(root, text="Ver Gráfico", command=ver_grafico, font=("Arial", 12), bg="#2196F3", fg="white").pack()
...
# Contador
contador_label = tk.Label(root, text="", font=("Arial", 11, "italic"), fg="black")
contador_label.pack()
